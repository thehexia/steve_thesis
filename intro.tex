...
This model cannot last forever. Future networks will require that network flows be directed in increasingly complex ways. More protocols and information will be carried on packets than ever before, and the modern switch model is not flexible enough.
...
SDN modifies two major elements of a conventional network switch: the data plane and the control plane. The \emph{data plane}, sometimes called the \emph{forwarding plane}, is the element which is responsible for forwarding packets between ports on the device. The control plane manages the data plane. It is the element which learns and configures the forwarding logic used by the data plane. It is also capable of processing packets which the data plane cannot.
...
efficient network functions implemented in software.

The conventional control plane is replaced by a software application known as a \emph{controller} (software control plane).
This controller then serves as the network application which manages the switch's behavior. 
...
The \emph{programmable data plane} remains on the device and is implemented with specialized, programmable packet processors. 
...
FFVM serves as the Steve runtime environment \cite{freeflow_software}.
