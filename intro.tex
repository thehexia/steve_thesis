...
The \emph{programmable data plane} remains on the device and is implemented with specialized, programmable processors. Software may then be installed to program this hardware.
...
\cite{llvm_webpage}. The LLVM IR optimizing compiler is able to produce code equivalent to that of a similar C implementation (as Clang \cite{clang_webpage} also uses LLVM). The LLVM infrastructure can also be modified to support special intrinsics which can be optimized for specific switch architectures. Right now, it targets an x86 architecture.
...
Chapter \ref{ch:limits} elaborates on pipeline safety guarantees and what algorithms enforce them
as well as providing the proofs for these algorithms. It also provides explanations about other 
