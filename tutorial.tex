...
exact_table ip_proto(ipv4.fragment, ipv4.protocol)
	requires (ipv4.ttl)
{  
  // Here we have 0x0 for the fragment field
...
Flow entries may also be given a \textit{properties}. Properties are additionally information stored alongside flow entries. Steve supports two properties: timeout and egress.
...
\textit{What makes using a table different from decision structures like an if-else or match statements?} As anyone can tell, the table presented in Section \ref{tut:basic_table} could have been written as a simple match statement. What are the advantages?

\textit{Tables can match on one or more fields at once.} The more fields required in the decision making process, the more complex using nested decision structures gets. Tables can also match on a packet's \texttt{in\_port} (input port) and \texttt{in\_phys\_port} (physical input port) fields (described in Section \ref{tut:output_action}).
