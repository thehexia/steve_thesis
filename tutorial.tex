...
written in Steve -- a simple MAC learning switch, also known as a bridge.
...
After the first example, this chapter will delve into all the individual pieces of a Steve program with greater detail.
...
ethernet header, it will need an ethernet decoder. 
There is no reason to extract \texttt{ethernet.type} because this application does not need to know what the Layer 3 protocol is.
...
After that, the bridge will need flow tables, the topic of Section \ref{tut:table}, for making forwarding decisions.
...
Within the body of the event handler, \texttt{ethernet.src} is used like a variable name. All extracted fields can be used this way inside decoders, tables/flow entries, and event handlers under certain circumstances described throughout this chapter.
...
\subsection{Common Statements} \label{tut:condition}
...
An \textit{event handler} is a special processing stage executed in the control plane outside the regular
run-to-completion pipeline. They are special
...
Each filtering table will only allow packets whose destination ports are \texttt{80} (HTTP) and \texttt{443} (HTTPS) to be forwarded. All others logged by an event and dropped. Additional flow entries may be added depending on the purpose of the firewall. It is best practice to defined the minimum \textit{allowed} ports rather than those ports that need blocked. From here, the firewall will send to the learning and forwarding tables defined earlier in Section \ref{tut:learning_router}.
