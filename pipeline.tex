...
describes the abstract mechanisms supported by the language and, by extension, features which must
be supported by an SDN switch that can run a Steve application.
Specifically, this chapter describes:

\begin{enumerate}
\item how a Steve program divides between data and control planes,
\item what operations these two elements must support, and
...
Because pipeline stages do not store any stateful information, they use this data structure to learn about contextual information about the packet.
...
There are safety issues associated with reinterpreting foreign allocated memory
...
A common set of actions, described in Section \ref{action_desc}, are then applied to classified packets.
...
expected to support a limited common set of \texttt{actions}.
...
\item \textbf{Set.} This is used to modify a field in the packet. This action enforces that the same number of bytes being written matches the size of a field to prevent buffer overflowing.
...
it may execute a wider range of operations supported by general purpose processors (albeit slower) such as those found in a typical C program.

\section{Steve Compilation and Portability} \label{compile_desc}
...
A current concern with Steve is that we do not own the commodity SDN hardware that conforms to this abstract machine as they tend to be too expensive. We are also unaware of common capabilities for these devices as SDN architectures are exotic and differ wildly between vendors. It may be possible that certain elements in the abstract machine (such as functions and memory for contexts) are not supported by some hardware.