...
\begin{subfigure}[t]{.45\textwidth}
  ...
  \caption{A abstract binding environment example.}
  \label{fg:context1}
\end{subfigure}
\hfill
\begin{subfigure}[t]{.45\textwidth}
  ...
  \caption{The contents of an environment when decoding an IP-in-IP packet.}
  \label{fg:context2}
\end{subfigure}
...
\subsection{Decoding Stages} \label{decoder_desc}
...
identified by its match fields and its priority.
...
First, all actions written the context's action set are executed. Then the data
...
handled by an event handler executed by a controller in the control plane.
...
Steve provides both pipeline and control elements for the SDN switch
running Freeflow.