...
By the end of this chapter, a user should be able to write many basic network applications using Steve. Two such applications will be presented; a simple learning switch and a simple learning router. Each example will be composed of smaller components described in this tutorial. To skip directly to these examples, move to Section \ref{examples_tut}.
...
First of all, a table can match on one or more fields at once. The table can also match on a packet's \texttt{in\_port} and \texttt{in\_phys\_port} fields (described in Section \ref{output_action_tut}. The more fields required in the decision making process, the more complex using nested decision structures get.
...
Tables have access to every field that has been extracted before them using \textbf{field access expressions}. The only limitation is that field access can only occur on fields which the table either matches on or requires explicitly.
...
Inserting flow entries into a table is a Steve action not explicitly supported by the OpenFlow standard \cite{openflow_spec} thought it is supported in software switches like OVS \cite{ovs_man_page}. Flow entries can be inserted with constant key values and no properties. 
...
Another thing to note is that this usage of events to learn flow entries is distinct from the OpenFlow paradigm. Normally packets are sent to the \texttt{controller} port which connects to an application called the \textbf{controller}. The controller then asynchronously processes packets and potentially adds the flow entries.

Steve events are written as part of the Steve application and are thus subject to the same semantic, logical, and safety guarantees applied to other pipeline stages. 
...
Let's begin by writing the Layer 2 (ethernet) learning switch. The learning switch shall receive packets. With each packet, the switch will "learn" which MAC addresses are found on which ports by listening to the \texttt{src} field of ethernet headers. The switch forwards packets by looking at the \texttt{dst} field and checking if it has learned that MAC address yet. If it has not learned the address, it floods the packet.

Here we need is the ethernet header, \texttt{src}, and \texttt{dst} fields. No other headers are necessary. First, we declare the layout needed to decode. Here we can use the layout given earlier in Figure \ref{fg:eth_layout_ex}.

Next we produce the ethernet decoder. We only care about the contents of the ethernet header so it will not be necessary to extract \texttt{eth.type} or proceed to another decoder. Instead, the decoder goes immediately to the \texttt{learn} table.
...
The forwarding table ultimately forwards the packet to the correct port. The forwarding table matches on the \texttt{ethernet.dst} field for the packet.
...
So how do we write the \texttt{learn\_mac} event? Recall that we will need the \texttt{insert} action. Additionally, this event will require that \texttt{ethernet.src} have been extracted. First we must insert a flow entry into the \texttt{learn} table with the \texttt{ethernet.src} field value of the current packet being learned. This prevents packets with the same \texttt{ethernet.src} field from triggering the \texttt{learn\_mac} event more than once. 
...
First we need the IPv4 header. Here we can use the layout given in Figure \ref{fg:ipv4_layout_ex}. We will also need the ethernet header from the prior example. The \texttt{ethernet.type} field confirms that the packet contains IPv4. Our decoders will look like the following.
...
We will need two tables just like in the learning switch example. Here the \texttt{learn} table matches on \texttt{ipv4.src}. The the \texttt{forward} table matches on \texttt{ipv4.dst}. Also all flow entries decrement the \texttt{ipv4.ttl} field.
