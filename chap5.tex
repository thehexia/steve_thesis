...
In the second form \grd{field}{name}.\grd{field}{id}, or more succinctly \texttt{F.E}, \texttt{F} must be a valid field name which refers to a field declaration whose type, \texttt{T}, is layout type (see \ref{layout_type_guide}). \texttt{E} must be found in the layout scope of the layout declaration declaring type \texttt{T} using a qualified name lookup.

The \textit{containing layout identifier} is the phrase used to mean the leftmost layout identifier in a field name, that is, \grd{layout}{id}.

Field names used as expressions become \textit{field access expressions} (see \ref{field_access_expr_guide}). Field access expressions may be used to mean the last extraction of that field name. Though field access expressions are parsed as \textit{postfix expressions} (see \ref{postfix_expr_guide}), they are made linear by the language to be \textit{primary expressions}.
...
The containing layout identifier of \grd{field}{name} (see \ref{field_name_guide}) shall be the same layout identifier as the one given by the decoder's \textit{layout rule} (see \ref{decoder_guide}).

\subsection{Determining the Location and Length of Extracted Fields}

Extract declarations execute a deterministic set of instructions to extract a field and save its location and length in the \textit{context} (see \ref{context_guide}). 
