 ...
 \Function{DFS}{$G, v, p$}
 	...
 	\If{meetsRequirements(v, p)}
 		\For{\textit{all} a \textit{in} G.adjacentNodes(v)}
 			\If{(a.visited == false) $\land$ canProgress(v, a)}
...
\textit{Termination Proof.} This algorithm is guaranteed to terminate on DAGs. By definition, a DAG has a finite number of unique paths that can be taken from source to sink. A unique path, by definition, is an ordered sequence of nodes that is distinct from all other paths.
Now, with proof by contradiction, assume the algorithm might not terminate. This would imply that this algorithm can visit the same unique path more than once.
Let $V$ be the set of all nodes in the graph $G$.
For a graph traversal algorithm to visit the same path more than once, for each $v \in V$, it must be possible to traverse from $v$ to a node adjacent to $v$ more than once. 

In this algorithm, given any node, $v$, in the graph there is a set of adjacent nodes, $A$. When this algorithm traverses the graph, for each node $a \in A$, the algorithm traverses the edge from $v$ to $a$ exactly once. If this algorithm does not terminate, then the algorithm must be able to traverse from $v$ to any node $a \in A$ more than once. But the algorithm explicitly prevents this, therefore the assumption is false and the algorithm does terminate.
